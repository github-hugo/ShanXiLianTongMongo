...
import nhb.system.platform.dataaccess.service.device.DeviceTypeService;
...
public class ReportApi {
	...
	public RestResultDto findEnergyByDeviceType(@RequestBody ReportRequest request) {
		...
		try {
			...
			for (SysArea area : sysAreas) {
				...
				childDeviceIds = commonService.treeAreaList(area.getId(), deviceIdsChild);
			...
			if (sysUser.getRole().equals(SysUserRoleEnum.PRIMARY_USER.getKey())
					|| sysUser.getRole().equals(SysUserRoleEnum.SECONDARY_USER.getKey())
					|| sysUser.getRole().equals(SysUserRoleEnum.COMMON_USER.getKey())) {
	...
	public RestResultDto findAvaMonthEnergy(@RequestBody ReportRequest request) {
		...
		try {
			...
			// 查询 电能汇总表
			List<EnergyCusume> cusumesList = energyCusumeService.findByDeviceIdInAndDateBetween(deviceIds, startTime,
					endTime);
