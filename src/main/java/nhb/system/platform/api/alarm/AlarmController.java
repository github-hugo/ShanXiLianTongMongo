package nhb.system.platform.api.alarm;
...
import nhb.system.platform.dataaccess.service.alarm.AlarmRuleService;
import nhb.system.platform.dataaccess.service.alarm.AlarmService;
...
import nhb.system.platform.enums.AlarmTypeEnum;
import nhb.system.platform.enums.SysUserRoleEnum;
import nhb.system.platform.request.alarm.AlarmRequest;
...
public class AlarmController {
    ...
    public RestResultDto findAlarmCounts(@RequestBody AlarmRequest request) {
        ...
        try {
            ...
            long unhandleAlarmCounts = alarmService.countByDeviceIdInAndDealStatusAndAlarmTimeBetween(deviceIds, 0, startTime, endTime);
            long alarmCounts = alarmService.countByDeviceIdInAndAlarmTimeBetween(deviceIds, startTime, endTime);
    ...
    public List<String> getDeviceIdsByUserId(String userId) {
        ...
        if (sysUser.getRole().equals(SysUserRoleEnum.ADMIN.getKey())) {
            List<ReceiptCollector> collectors = receiptCollectorService.findAllByManagerId(sysUser.getId());
            for (ReceiptCollector collector : collectors) {
                collectorIds.add(collector.getId());
            }
        } else {
            List<SysUserCollector> sysUserCollectors = sysUserCollectorService.findByUserId(userId);
    ...
    public RestResultDto findAlarms(@RequestBody AlarmRequest request) {
        ...
        try {
            ...
            Page<Alarm> pageList = alarmService.findByDeviceIdInAndDealStatusAndAlarmTimeBetween(deviceIds, dealStatus,
            ...
            if (null != pageList) {
                ...
                for (Object obj : pageList.getContent()) {
                    ...
                    mapValue.put("alarmTime",
                            DateUtil.format(new Date(Long.valueOf(String.valueOf(mapValue.get("alarmTime"))))));
    ...
    @RequestMapping(value = "findAlarmRuleByDeviceId", method = {RequestMethod.POST})
